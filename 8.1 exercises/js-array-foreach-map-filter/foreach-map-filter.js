...
function doubleValues(arr){   
    return arr.map(function(val){ 
        return val * 2; //return value times 2
    })
}
...
function extractFullName(arr){
    ...
    arr.forEach(function(val){
        ...
        flArray.push(`${val['first']} ${val['last']}`)
    })
    return(flArray);
}
...
function findInObj(arr, key, searchValue) {
     return arr.filter(function(val,i){
        // console.log(arr[i][key] == searchValue);
        if(arr[i][key] == searchValue){
            // console.log(val)
            return val
        };
    })[0] // Grabbing the first index of this new filtered array
}
...
function removeVowels(str) {
    return Array.from(str.toLowerCase()).filter(function(val){
        // console.log(val,'is NOT vowel:',!vowels.includes(val))
